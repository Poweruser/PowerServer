...
public class GamespyProtocol1Parser implements DataParserInterface {
    ...
    public MessageData parse(GameBase game, UDPMessage msg) throws ParserException {
        ...
        if(start < 0) { throw new ParserException("The message does not contain a backslash \"\\\". Is it a gamespy protocol version 2 message?", msg, game); }
        message = message.substring(start + 1);
        String[] split = message.split("\\\\");
        if((split.length % 2) != 0) { throw new ParserException("The number of elements in a gamespy protocol version 1 message must be even.", msg, game); }
        ...
        for(int i = 0; i < split.length - 1; i = i + 2) {
            ...
            if(game == null || game.verifyDataKeyAndValue(key, value)) {
                ...
                throw new ParserException("Found invalid data key \"" + key + "\" and value \"" + value + "\".", msg, game);
            }
        }
